import {
  ...
  splitSignature,
  stripZeros
} from "./chunk-6ZJG4LST.js";
import {
  require_hash
} from "./chunk-7PS6RHSY.js";
import {
  require_sha3
} from "./chunk-EDPTV5CR.js";
import {
  __esm,
  __export,
  __toESM
...
function _decodeChildren(data, offset, childOffset, length) {
  ...
  while (childOffset < offset + 1 + length) {
    ...
    if (childOffset > offset + 1 + length) {
      logger2.throwError("child data too short", Logger.errors.BUFFER_OVERRUN, {});
    }
  }
...
function _decode(data, offset) {
  if (data.length === 0) {
    logger2.throwError("data too short", Logger.errors.BUFFER_OVERRUN, {});
  }
  if (data[offset] >= 248) {
    const lengthLength = data[offset] - 247;
    if (offset + 1 + lengthLength > data.length) {
      logger2.throwError("data short segment too short", Logger.errors.BUFFER_OVERRUN, {});
    }
    const length = unarrayifyInteger(data, offset + 1, lengthLength);
    if (offset + 1 + lengthLength + length > data.length) {
      logger2.throwError("data long segment too short", Logger.errors.BUFFER_OVERRUN, {});
    }
    return _decodeChildren(data, offset, offset + 1 + lengthLength, lengthLength + length);
  } else if (data[offset] >= 192) {
    const length = data[offset] - 192;
    if (offset + 1 + length > data.length) {
      logger2.throwError("data array too short", Logger.errors.BUFFER_OVERRUN, {});
    }
...
var logger3, MAX_SAFE_INTEGER, ibanLookup, safeDigits;
var init_lib7 = __esm({
  "../../node_modules/@ethersproject/address/lib.esm/index.js"() {
    "use strict";
    init_lib2();
...
var logger4, UnicodeNormalizationForm, Utf8ErrorReason, Utf8ErrorFuncs;
var init_utf8 = __esm({
  "../../node_modules/@ethersproject/strings/lib.esm/utf8.js"() {
    "use strict";
    init_lib2();
...
function checkString(key2) {
  return function(value) {
    if (typeof value !== "string") {
      logger6.throwArgumentError(`invalid domain value for ${JSON.stringify(key2)}`, `domain.${key2}`, value);
    }
    return value;
  };
}
function getBaseEncoder(type) {
  {
    ...
    if (match) {
      ...
      if (width % 8 !== 0 || width > 256 || match[2] && match[2] !== String(width)) {
        logger6.throwArgumentError("invalid numeric width", "type", type);
      }
      ...
      return function(value) {
        ...
        if (v.lt(boundsLower) || v.gt(boundsUpper)) {
          logger6.throwArgumentError(`value out-of-bounds for ${type}`, "value", value);
        }
  ...
  }
  {
    ...
    if (match) {
      ...
      if (width === 0 || width > 32 || match[1] !== String(width)) {
        logger6.throwArgumentError("invalid bytes width", "type", type);
      }
      return function(value) {
        const bytes = arrayify(value);
        if (bytes.length !== width) {
          logger6.throwArgumentError(`invalid length for ${type}`, "value", value);
        }
...
var __awaiter2, logger6, padding, NegativeOne2, Zero2, One2, MaxUint2562, hexTrue, hexFalse, domainFieldTypes, domainFieldNames, domainChecks, TypedDataEncoder;
  ...
  "../../node_modules/@ethersproject/hash/lib.esm/typed-data.js"() {
    ...
    domainChecks = {
      ...
      chainId: function(value) {
        ...
        return logger6.throwArgumentError(`invalid domain value for "chainId"`, "domain.chainId", value);
      },
      verifyingContract: function(value) {
        ...
        return logger6.throwArgumentError(`invalid domain value "verifyingContract"`, "domain.verifyingContract", value);
      },
      salt: function(value) {
        try {
          const bytes = arrayify(value);
        ...
        return logger6.throwArgumentError(`invalid domain value "salt"`, "domain.salt", value);
      }
    };
    TypedDataEncoder = class {
      constructor(types) {
        ...
        for (const name in types) {
          ...
          types[name].forEach((field) => {
            if (uniqueNames[field.name]) {
              logger6.throwArgumentError(`duplicate variable name ${JSON.stringify(field.name)} in ${JSON.stringify(name)}`, "types", types);
            }
            uniqueNames[field.name] = true;
            const baseType = field.type.match(/^([^\x5b]*)(\x5b|$)/)[1];
            if (baseType === name) {
              logger6.throwArgumentError(`circular type reference to ${JSON.stringify(baseType)}`, "types", types);
            }
            ...
            if (!parents[baseType]) {
              logger6.throwArgumentError(`unknown type ${JSON.stringify(baseType)}`, "types", types);
            }
        ...
        if (primaryTypes.length === 0) {
          logger6.throwArgumentError("missing primary type", "types", types);
        } else if (primaryTypes.length > 1) {
          logger6.throwArgumentError(`ambiguous primary types or unused types: ${primaryTypes.map((t) => JSON.stringify(t)).join(", ")}`, "types", types);
        }
        defineReadOnly(this, "primaryType", primaryTypes[0]);
        function checkCircular(type, found) {
          if (found[type]) {
            logger6.throwArgumentError(`circular type reference to ${JSON.stringify(type)}`, "types", types);
          }
          found[type] = true;
      ...
      _getEncoder(type) {
        ...
        if (match) {
          ...
          return (value) => {
            if (length >= 0 && value.length !== length) {
              logger6.throwArgumentError("array length mismatch; expected length ${ arrayLength }", "value", value);
            }
        ...
        return logger6.throwArgumentError(`unknown type: ${type}`, "type", type);
      }
      encodeType(name) {
        const result = this._types[name];
        if (!result) {
          logger6.throwArgumentError(`unknown type: ${JSON.stringify(name)}`, "name", name);
        }
        return result;
      }
      ...
      static hashDomain(domain) {
        ...
        for (const name in domain) {
          ...
          if (!type) {
            logger6.throwArgumentError(`invalid typed-data domain key: ${JSON.stringify(name)}`, "domain", domain);
          }
      ...
      static getPayload(domain, types, value) {
        ...
        if (typesWithDomain.EIP712Domain) {
          logger6.throwArgumentError("types must not contain EIP712Domain type", "types.EIP712Domain", types);
        } else {
            ...
            switch (type) {
              ...
              case "string":
                if (typeof value2 !== "string") {
                  logger6.throwArgumentError(`invalid string`, "value", value2);
                }
                return value2;
            }
            return logger6.throwArgumentError("unsupported type", "type", type);
          })
        };
      }
    };
...
var __awaiter3, logger7, ForkEvent, Provider;
var init_lib13 = __esm({
  "../../node_modules/@ethersproject/abstract-provider/lib.esm/index.js"() {
    ...
    Provider = class {
      constructor() {
        logger7.checkAbstract(new.target, Provider);
...
var init_version7 = __esm({
  "../../node_modules/@ethersproject/abstract-signer/lib.esm/_version.js"() {
    version7 = "abstract-signer/5.7.0";
  }
});

// ../../node_modules/@ethersproject/abstract-signer/lib.esm/index.js
var __awaiter4, logger8, allowedTransactionKeys, forwardErrors, Signer, VoidSigner;
var init_lib14 = __esm({
  "../../node_modules/@ethersproject/abstract-signer/lib.esm/index.js"() {
    ...
    Signer = class {
      constructor() {
        logger8.checkAbstract(new.target, Signer);
      ...
      checkTransaction(transaction) {
        for (const key2 in transaction) {
          if (allowedTransactionKeys.indexOf(key2) === -1) {
            logger8.throwArgumentError("invalid transaction key: " + key2, "transaction", transaction);
          }
        }
        ...
        if (tx.from == null) {
          ...
          tx.from = Promise.all([
            ...
            if (result[0].toLowerCase() !== result[1].toLowerCase()) {
              logger8.throwArgumentError("from address mismatch", "transaction", transaction);
            }
            return result[0];
          });
        }
        ...
        return __awaiter4(this, void 0, void 0, function* () {
          ...
          if (tx.to != null) {
            tx.to = Promise.resolve(tx.to).then((to) => __awaiter4(this, void 0, void 0, function* () {
              ...
              if (address == null) {
                logger8.throwArgumentError("provided ENS name resolves to null", "tx.to", to);
              }
              return address;
            }));
            tx.to.catch((error) => {
          ...
          if (tx.gasPrice != null && (tx.type === 2 || hasEip1559)) {
            logger8.throwArgumentError("eip-1559 transaction do not support gasPrice", "transaction", transaction);
          } else if ((tx.type === 0 || tx.type === 1) && hasEip1559) {
            logger8.throwArgumentError("pre-eip-1559 transaction do not support maxFeePerGas/maxPriorityFeePerGas", "transaction", transaction);
          }
          if ((tx.type === 2 || tx.type == null) && (tx.maxFeePerGas != null && tx.maxPriorityFeePerGas != null)) {
            ...
            if (tx.type == null) {
              if (feeData.maxFeePerGas != null && feeData.maxPriorityFeePerGas != null) {
                ...
                if (hasEip1559) {
                  logger8.throwError("network does not support EIP-1559", Logger.errors.UNSUPPORTED_OPERATION, {
                ...
                logger8.throwError("failed to get consistent fee data", Logger.errors.UNSUPPORTED_OPERATION, {
          ...
          if (tx.gasLimit == null) {
            tx.gasLimit = this.estimateGas(tx).catch((error) => {
              ...
              return logger8.throwError("cannot estimate gas; transaction may fail or may require manual gas limit", Logger.errors.UNPREDICTABLE_GAS_LIMIT, {
                error,
                tx
              });
            });
          ...
          if (tx.chainId == null) {
            ...
            tx.chainId = Promise.all([
              ...
              if (results[1] !== 0 && results[0] !== results[1]) {
                logger8.throwArgumentError("chainId address mismatch", "transaction", transaction);
              }
              return results[0];
            });
          }
      ...
      _checkProvider(operation) {
        if (!this.provider) {
          logger8.throwError("missing provider", Logger.errors.UNSUPPORTED_OPERATION, {
    ...
    VoidSigner = class extends Signer {
      ...
      _fail(message, operation) {
        return Promise.resolve().then(() => {
          logger8.throwError(message, Logger.errors.UNSUPPORTED_OPERATION, { operation });
        });
      }
      signMessage(message) {
...
function formatNumber(value, name) {
  ...
  if (result.length > 32) {
    logger10.throwArgumentError("invalid length for " + name, "transaction:" + name, value);
  }
  return result;
}
  ...
  return {
    ...
    storageKeys: (storageKeys || []).map((storageKey, index) => {
      if (hexDataLength(storageKey) !== 32) {
        logger10.throwArgumentError("invalid access list storageKey", `accessList[${addr}:${index}]`, storageKey);
      }
  ...
  if (Array.isArray(value)) {
    return value.map((set, index) => {
      if (Array.isArray(set)) {
        if (set.length > 2) {
          logger10.throwArgumentError("access list expected to be [ address, storageKeys[] ]", `value[${index}]`, set);
        }
...
function _serializeEip1559(transaction, signature2) {
  if (transaction.gasPrice != null) {
    ...
    if (!gasPrice.eq(maxFeePerGas)) {
      logger10.throwArgumentError("mismatch EIP-1559 gasPrice != maxFeePerGas", "tx", {
        gasPrice,
        maxFeePerGas
      });
    }
...
function _serialize(transaction, signature2) {
  ...
  transactionFields.forEach(function(fieldInfo) {
    ...
    if (fieldInfo.length && value.length !== fieldInfo.length && value.length > 0) {
      logger10.throwArgumentError("invalid length for " + fieldInfo.name, "transaction:" + fieldInfo.name, value);
    }
  ...
  if (transaction.chainId != null) {
    ...
    if (typeof chainId !== "number") {
      logger10.throwArgumentError("invalid transaction.chainId", "transaction", transaction);
    }
  ...
  if (chainId !== 0) {
    ...
    v += chainId * 2 + 8;
    if (sig.v > 28 && sig.v !== v) {
      logger10.throwArgumentError("transaction.chainId/signature.v mismatch", "signature", signature2);
    }
  } else if (sig.v !== v) {
    logger10.throwArgumentError("transaction.chainId/signature.v mismatch", "signature", signature2);
  }
...
function serialize(transaction, signature2) {
  if (transaction.type == null || transaction.type === 0) {
    if (transaction.accessList != null) {
      logger10.throwArgumentError("untyped transactions do not support accessList; include type: 1", "transaction", transaction);
    }
  ...
  return logger10.throwError(`unsupported transaction type: ${transaction.type}`, Logger.errors.UNSUPPORTED_OPERATION, {
...
function _parseEipSignature(tx, fields, serialize2) {
  try {
    ...
    logger10.throwArgumentError("invalid v for transaction type: 1", "v", fields[0]);
  }
...
function _parseEip1559(payload) {
  ...
  if (transaction.length !== 9 && transaction.length !== 12) {
    logger10.throwArgumentError("invalid component count for transaction type: 2", "payload", hexlify(payload));
  }
...
function _parseEip2930(payload) {
  ...
  if (transaction.length !== 8 && transaction.length !== 11) {
    logger10.throwArgumentError("invalid component count for transaction type: 1", "payload", hexlify(payload));
  }
  const tx = {
    type: 1,
    chainId: handleNumber(transaction[0]).toNumber(),
...
function _parse(rawTransaction) {
  ...
  if (transaction.length !== 9 && transaction.length !== 6) {
    logger10.throwArgumentError("invalid raw transaction", "rawTransaction", rawTransaction);
  }
  const tx = {
...
function parse(rawTransaction) {
  ...
  return logger10.throwError(`unsupported transaction type: ${payload[0]}`, Logger.errors.UNSUPPORTED_OPERATION, {
    operation: "parseTransaction",
    transactionType: payload[0]
  });
}
var logger10, TransactionTypes, transactionFields, allowedTransactionKeys2;
...
function computeHmac(algorithm, key2, data) {
  if (!SupportedAlgorithm[algorithm]) {
    logger11.throwError("unsupported algorithm " + algorithm, Logger.errors.UNSUPPORTED_OPERATION, {
      operation: "hmac",
      algorithm
    });
  }
...
export {
  ...
  TypedDataEncoder,
  lib_exports8,
  init_lib12 as init_lib8,
  ForkEvent,
  Provider,
  init_lib13 as init_lib9,
  Signer,
  VoidSigner,
  init_lib14 as init_lib10,
  SigningKey,
  recoverPublicKey,
  computePublicKey,
  lib_exports9,
  init_lib15 as init_lib11,
  TransactionTypes,
  computeAddress,
  recoverAddress,
  accessListify,
  ...
  computeHmac,
  lib_exports12,
  init_lib18 as init_lib14,
  randomBytes,
  shuffled,
  lib_exports13,
  init_lib19 as init_lib15,
  _fetchData,
  fetchJson,
  poll,
  lib_exports14,
  init_lib20 as init_lib16
};
//# sourceMappingURL=chunk-5FB5HXLZ.js.map
